...
WbDeviceTag Y_motor_pos_sensor;
/*
函数功能：初始化devices
*/
...
{
    // get device
    spring_motor = wb_robot_get_device("z_motor");
    spring_pos_sensor = wb_robot_get_device("z_sensor");
    touch_sensor = wb_robot_get_device("foot_sensor");
    IMU = wb_robot_get_device("imu");
    X_motor = wb_robot_get_device("x_motor");
    Y_motor = wb_robot_get_device("y_motor");
    X_motor_pos_sensor = wb_robot_get_device("x_sensor");
    Y_motor_pos_sensor = wb_robot_get_device("y_sensor");
    // enable
    wb_position_sensor_enable(spring_pos_sensor, TIME_STEP);
    wb_position_sensor_enable(X_motor_pos_sensor, TIME_STEP);
    wb_position_sensor_enable(Y_motor_pos_sensor, TIME_STEP);
...
/*
函数功能：设置虚拟弹簧的力
注    意：弹簧力正方向定义为：腿不动，将机身向z轴方向推动的力
*/
...
*/
void set_Y_torque(double torque)
{
    wb_motor_set_torque(Y_motor, torque);
}
//-----------------------------------------------------------sensor
/*
函数功能：获取弹簧长度
...
函数功能：获取 Z 轴电机角度,角度制
*/
double get_Y_motor_angle()
{
    double angle = wb_position_sensor_get_value(Y_motor_pos_sensor);
    return angle * 180.0f / PI;
}
/*
函数功能：检测足底是否接触地面
